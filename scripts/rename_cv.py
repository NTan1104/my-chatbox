...
def rename_pdfs_to_cv(source_dirs: list[Path], start_index: int = 1) -> None:
	pdf_files = collect_pdfs(source_dirs)

	if not pdf_files:
		print("Không tìm thấy file PDF nào để đổi tên.")
		return

	temp_targets: list[tuple[Path, Path]] = []
	for index, pdf_path in enumerate(pdf_files, start=start_index):
		temp_name = f"__tmp_cv_{uuid.uuid4().hex}_{index}{pdf_path.suffix}"
	...
	for index, (_, temp_path) in enumerate(temp_targets, start=start_index):
		final_name = f"CV_{index:03d}{temp_path.suffix.lower()}"
